...
st.set_page_config(page_title="Plot report")
...
if "dataset" not in st.session_state:
    ...
    # Supponiamo che questo sia il tuo dizionario o lista di colonne
    # if grafici == 'anagrafiche':
    #     colonne0 = ['SESSO', 'TITOLO_DI_STUDIO', 'GENERAZIONE'] # Esempio con più colonne
        
    #     # 1. Definiamo il numero di colonne della griglia (nel tuo caso 2)
    #     N_COLONNE_GRIGLIA = 2
        
    #     # 2. Cicliamo sulle colonne del dataframe a gruppi di 2
    #     for i in range(0, len(colonne0), N_COLONNE_GRIGLIA):
            
    #         # Creiamo un "blocco" riga con un container
    #         with st.container():
    #             # Creiamo le 2 colonne di Streamlit all'interno del container
    #             cols = st.columns(N_COLONNE_GRIGLIA)
                
    #             # Gestiamo la prima colonna della riga corrente
    #             if i < len(colonne0):
    #                 colonna_df = colonne0[i]
    #                 with cols[0]:
    #                     # Creiamo un container specifico per questo grafico
    #                     with st.container(border=True): # border=True aggiunge un bel riquadro (Streamlit >= 1.29)
    #                         st.subheader(f"Distribuzione {colonna_df.replace('_', ' ').title()}")
                            
    #                         # Grafico Matplotlib singolo (NON subplot)
    #                         fig, ax = plt.subplots(figsize=(5, 4))
    #                         tab = df[colonna_df].value_counts()
    #                         ax.pie(tab, labels=tab.index, autopct='%1.1f%%', startangle=90)
                            
    #                         # Mostriamo il grafico nel container
    #                         st.pyplot(fig)
    #                         plt.close(fig) # Ottimizza la memoria liberando il grafico chiuso
                
    #             # Gestiamo la seconda colonna della riga corrente (se esiste)
    #             if i + 1 < len(colonne0):
    #                 colonna_df = colonne0[i + 1]
    #                 with cols[1]:
    #                     with st.container(border=True):
    ...
    #                         plt.close(fig)
    # else:
    #     st.warning("Seleziona un grafico valido.")